...
public class SqueezingRecipeTrigger extends SimpleCriterionTrigger<SqueezingRecipeTrigger.TriggerInstance> {
    @Override
    ...
    public record TriggerInstance(Optional<ContextAwarePredicate> player, Optional<ItemPredicate> inputItem, Optional<ItemPredicate> outputItem) implements SimpleInstance {
        ...
        public static Criterion<TriggerInstance> squeezing(Item input, Item output) {
            return squeezing(ItemPredicate.Builder.item().of(input), ItemPredicate.Builder.item().of(output));
        }

        public static Criterion<TriggerInstance> squeezing(ItemPredicate.Builder input, ItemPredicate.Builder output) {
            return ModCriterionTriggers.SQUEEZING.get().createCriterion(new TriggerInstance(Optional.empty(), Optional.of(input.build()), Optional.of(output.build())));
        }
