...
public class PlacerPlaceBlockTrigger extends SimpleCriterionTrigger<PlacerPlaceBlockTrigger.TriggerInstance> {
    @Override
    public Codec<PlacerPlaceBlockTrigger.TriggerInstance> codec() {
    ...
    public record TriggerInstance(Optional<ContextAwarePredicate> player, Optional<BlockPredicate> block) implements SimpleCriterionTrigger.SimpleInstance {
        ...
        public static Criterion<TriggerInstance> placeBlock(Block block) {
            return placeBlock(BlockPredicate.Builder.block().block(block));
        }
