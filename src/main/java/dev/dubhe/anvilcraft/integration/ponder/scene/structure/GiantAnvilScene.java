...
public class GiantAnvilScene {
    ...
    private static void shock(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        builder.effects()
            .emitParticles(
                util.vector().of(17, 1, 12),
                builder.effects().simpleParticleEmitter(ParticleTypes.POOF, util.vector().of(0, 0.1, 0)),
                10,
                1
            );
        //XeKr红温倒地
    ...
    private static ElementLink<EntityElement> spawnMushroomCow(SceneBuildingUtil util, AnvilCraftSceneBuilder builder) {
        return builder.world().createEntity(w -> {
            ...
            Vec3 p = util.vector().topOf(util.grid().at(17, 1, 12));
    ...
    private static void shockRange(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        builder.effects()
            .emitParticles(
                util.vector().of(17, 1, 10),
                scene.effects().simpleParticleEmitter(ParticleTypes.POOF, util.vector().of(0, 0.1, 0)),
                10,
                1
            );
        //XeKr红温倒地
    ...
    private static void shockMode(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        ElementLink<EntityElement> zombie = builder.world().createEntity(w -> {
            net.minecraft.world.entity.monster.Zombie entity = EntityType.ZOMBIE.create(w);
            Vec3 p = util.vector().topOf(util.grid().at(17, 1, 12));
            entity.setPos(p.x, p.y, p.z);
            entity.xo = p.x;
            entity.yo = p.y;
        ...
        for (int i = 1; i <= 20; i++) {
            ...
            builder.world().modifyEntity(
                zombie, e -> {
                    if (e instanceof net.minecraft.world.entity.monster.Zombie zombieEntity) {
                        zombieEntity.deathTime = tick;
                    }
                }
            );
        }
