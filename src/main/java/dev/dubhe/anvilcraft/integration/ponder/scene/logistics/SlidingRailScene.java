...
public class SlidingRailScene {
    ...
    private static void blockSliding(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        // 在滑轨上放置方块
        scene.world().setBlock(slimePos, Blocks.GLASS.defaultBlockState(), false);
        ElementLink<WorldSectionElement> glass = scene.world().showIndependentSection(util.select().position(slimePos), Direction.DOWN);
        scene.idle(5);
        ...
        scene.world().moveSection(glass, offset, 2);
        scene.idle(4);

        // 其他方块继续滑动
        scene.world().moveSection(glass, new Vec3(-distance, 0, 0), (int) (distance / SlidingBlockEntity.DEFAULT_MOVEMENT));
        scene.idle(30);

        // 移除其他方块
        scene.world().hideIndependentSection(glass, Direction.UP);
        scene.idle(20);
        ...
        ElementLink<WorldSectionElement> structure = scene.world().showIndependentSection(util.select().fromTo(slimePos, glassPos), Direction.DOWN);
        scene.idle(5);
        ...
        // 移除结构
        scene.world().hideIndependentSection(structure, Direction.UP);
        scene.idle(20);
        ...
        // 放置铁砧
        scene.world().setBlock(slimePos, Blocks.ANVIL.defaultBlockState(), false);
        scene.world().showSection(util.select().position(slimePos), Direction.DOWN);
        scene.idle(10);
